...
{
    if (findHallIndexById(hallId) != -1) 
    {
        std::cout << "\nBłąd: Sala o takim ID już istnieje.\n\n";
        return;
    }
...
int Cinema::addScreening(const std::string &title, const std::string &hallId, int startTimeMin, double price)
{
    ...
    if (movieId == -1) 
    {
        std::cout << "\nBłąd: Nie ma filmu o tytule: " << title << "\n\n";
        return -1;
    }
    ...
    for (size_t i = 0; i < screenings.size(); i++) 
    {
        ...
        {
            std::cout << "\nBłąd: W tej sali jest juz seans na dziś.\n\n";
            return -1;
        }
    }
...
void Cinema::startDay()
{
    if (dayActive) 
    {
        std::cout << "\nBłąd: Dzień juz jest aktywny.\n\n";
        return;
    }
    
    if (screenings.empty()) 
    {
        std::cout << "\nBłąd: Brak seansów. Najpierw ustaw repertuar.\n\n";
        return;
    }
...
void Cinema::advanceTime(int minutes)
{
    if (!dayActive) 
    {
        std::cout << "\nBłąd: Nie można zmieniać czasu, gdy dzień nie jest aktywny.\n\n";
        return;
    }
...
void Cinema::printSeatsByTitle(const std::string &title, const std::string &hallId, int startTimeMin) const
{
    ...
    if (sIndex == -1) 
    {
        std::cout << "\nBłąd: Brak seansu dla tego filmu / sali / godziny.\n\n";
        return;
    }
...
void Cinema::printRefundQueue() const
{
    ...
    for (size_t i = 0; i < tickets.size(); i++) 
    {
        ...
        if (t.status == TICKET_REFUND_REQUESTED) 
        {
                                    ...
                                    const std::vector<std::string> &seats)
{
    ...
    if (count <= 0) {
        std::cout << "\nBłąd: Liczba biletów musi być większa niż 0.\n\n";
        return created;
    }
    ...
    for (int i = 0; i < count; i++) 
    {
        ...
        {
            if (!h.isSeatLabelValid(seatLabel)) 
            {
                std::cout << "\nBłąd: Niepoprawne miejsce: " << seatLabel << "\n\n";
                break;
            }
            
            seatIndex = h.seatLabelToIndex(seatLabel);
            
            if (s.isSeatTaken(seatIndex)) 
            {
                std::cout << "\nBłąd: Miejsce zajęte: " << seatLabel << "\n\n";
                break;
            }
        }
...
void Cinema::requestRefund(const std::string &nickname, int ticketId)
{
    if (!dayActive) 
    {
        std::cout << "\nBłąd: Zwroty możliwe tylko w trakcie dnia.\n\n";
        return;
    }
    ...
    if (t.ownerNickname != nickname) 
    {
        std::cout << "\nBłąd: To nie jest Twój bilet.\n\n";
        return;
    }
    
    if (t.status != TICKET_PURCHASED) 
    {
        std::cout << "\nBłąd: Bilet nie jest w stanie Purchased.\n\n";
        return;
    }
    ...
    if (minutesToStart <= REFUND_LIMIT_MIN) 
    {
        std::cout << "\nBłąd: Zbyt późno na zwrot biletu (<= 60 min do seansu).\n\n";
        return;
    }

    t.status = TICKET_REFUND_REQUESTED;
...
void Cinema::rateMovie(const std::string &nickname, const std::string &title, int stars)
{
    ...
    if (mIndex == -1) 
    {
        std::cout << "\nBłąd: Brak filmu o tytule: " << title << "\n\n";
        return;
    }
    ...
    if (!hasTicket) 
    {
        std::cout << "\nBłąd: Nie masz żadnego biletu na film: " << title << "\n\n";
        return;
    }
...
void Cinema::decideRefund(int ticketId, bool approve)
{
    if (!dayActive) 
    {
        std::cout << "\nBłąd: Zwroty tylko w trakcie aktywnego dnia.\n\n";
        return;
    }
    ...
    if (t.status != TICKET_REFUND_REQUESTED) 
    {
        std::cout << "\nBłąd: Ten bilet nie oczekuje na zwrot.\n\n";
        return;
    }
    ...
    {
        t.status = TICKET_REFUND_REJECTED;
